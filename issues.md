...
| --- | --- | --- | --- | --- | --- | --- |
| 13 | [The 180s reviewer budget sits at the median review duration, so independent review usually cannot finish](#13-the-180s-reviewer-budget-sits-at-the-median-review-duration-so-independent-review-usually-cannot-finish) | Done (2026-08-29) | 8.0 (8×1÷1) | Sonnet 5 | Gemini 3 Pro | On HOWLFRAM-BUG-50, 13 of 20 reviewer attempts hit the 180s deadline and only one provider ever completed a review. Independent review collapsed onto the implementer because it was the only resource that fit the budget. |
| 14 | [Review failure evidence cannot distinguish a timeout from a quota failure from a launch failure](#14-review-failure-evidence-cannot-distinguish-a-timeout-from-a-quota-failure-from-a-launch-failure) | Done (2026-08-29) | 6.0 (6×1÷1) | Sonnet 5 | Gemini 3 Pro | When every reviewer candidate fails, `agent_result` is None, so `process`, `launch_outcome` and `normalized_failure` all persist as null and a 180s timeout is byte-identical to a 1.6s provider quota failure. |
| 16 | [`task_spec.actual_agent` lags one implementation attempt behind the resource that actually produced the candidate](#16-task_specactual_agent-lags-one-implementation-attempt-behind-the-resource-that-actually-produced-the-candidate) | Done (2026-08-30) | 4.0 (8×1÷2) | Sonnet 5 | Gemini 3 Pro | Root cause was not failover staleness: `actual_agent` was serving as both durable audit metadata and a per-invocation dispatch slot, and `invoke_reviewer_with_failover` wrote a *reviewer's* provider id into it. Split into `effective_implementer_resource_id` (authoritative) and `dispatch_resource_id` (transient). |
| 15 | [`classify_failure` reads Devin's quota exhaustion as an engineering failure, so the pool never marks it exhausted](#15-classify_failure-reads-devins-quota-exhaustion-as-an-engineering-failure-so-the-pool-never-marks-it-exhausted) | Done (2026-08-30) | 4.0 (8×1÷2) | Sonnet 5 | Gemini 3 Pro | Fixed in the shared classifier as proposed, by reading the structured `errorKind` envelope rather than matching prose. Two adjacent defects fixed with it: `QUOTA_EXHAUSTED` had no cooldown at all, and the reviewer launch budget was spent on providers already known to be dead. |
| 12 | [Approving a pre-verification escalation completes the task without ever running the deterministic verification plan](#12-approving-a-pre-verification-escalation-completes-the-task-without-ever-running-the-deterministic-verification-plan) | Done (2026-08-30) | 4.0 (8×1÷2) | Sonnet 5 | Gemini 3 Pro | Fix option 1 plus option 3, chosen together: approval now resumes into the deterministic gate, and a plan that does not pass reaches a terminal state that does not read as verified. No generic override was added. |
...
* **Status correction (2026-08-30):** Fixed by PR #61: `REVIEW_TIMEOUT_SECONDS = 600` in `review_runner.py`. The backlog row was not flipped at the time; corrected 2026-08-30 during a marathon-readiness audit.
...
* **Status correction (2026-08-30):** Fixed by PR #61: each reviewer attempt records `exit_code`, `timed_out`, `timeout_source`, `launch_outcome` and the normalized `failure_class`, and `write_review_result` falls back to the last recorded attempt when no `AgentExecutionResult` exists. The backlog row was not flipped at the time; corrected 2026-08-30 during a marathon-readiness audit.

### 16. `task_spec.actual_agent` lags one implementation attempt behind the resource that actually produced the candidate
* **Symptom:** on `HOWLFRAM-BUG-52`, three implementation attempts ran and all three exceeded their budget, after which a retained candidate was governed through review. The durable task spec recorded the wrong implementer:

  ```
  task.yaml            actual_agent:       codex          <- attempt 2
  effective_route.json selected_agent_id:  claude_code    <- attempt 3
  implementation/attempts/                 01-agy, 02-codex, 03-claude_code
  operator summary     Implementation:     claude_code
  reviews/*/result.json independence.implementer: claude_code   (10 of 10)
  ```

  Every artifact except `task.yaml` names `claude_code`. `actual_agent` is stale at the second of three attempts.
* **Root Cause (confirmed 2026-08-30; the suspicion below was disproved):** `actual_agent` was serving two incompatible purposes — durable audit metadata answering "who produced this work", and a mutable dispatch slot naming which resource the *next* backend invocation represents. `invoke_reviewer_with_failover` (`review_runner.py:563`) wrote the second meaning into the first, once per reviewer attempt. The value that survived was therefore written by a **review**, not by any implementation attempt.
* **How the two explanations were distinguished:** `codex` is genuinely implementation attempt 2, which is what made "lags one attempt behind" look plausible. But `codex` is also a reviewer on this run, and the review evidence settles it. Reviews ran in `recommended_reviewers` order — `correctness-reviewer`, `test-falsifier`, `simplicity-reviewer` — and the final cycle is `remediation/cycle-03/re_review/`, whose per-role attempt logs read:

  ```
  correctness-reviewer  attempts: agy(completed)
  test-falsifier        attempts: codex(reviewer_failure), agy(exhausted)
  simplicity-reviewer   attempts: devin_cli(reviewer_failure), codex(completed)   <- last write
  ```

  The last reviewer dispatched in the last cycle was `codex` serving `simplicity-reviewer`. That is the write that landed in `task.yaml`. Under the "stale failover" theory the value would have been fixed at implementation time and could not depend on review ordering at all.
* **Prior suspicion, now disproved:** that `actual_agent` was assigned during the failover loop and not refreshed for a final failed attempt whose artifact was retained as salvage. The orchestrator does track the true producer separately in `final_impl_resource_id`, and that is what reaches `ReviewRunner.execute_review_cycle` as `implementer_resource_id`, which is why review independence was computed correctly — but that is not what corrupted `actual_agent`.
* **A second defect found while fixing this:** `final_impl_resource_id` only ever became durable on the accepted path, via `_persist_effective_route(..., accepted=True)` at Stage 8. A task escalating before Stage 6 — which is exactly what HOWLFRAM-BUG-52 did — therefore recorded `accepted_implementation_resource: null` **and** `final_implementation_resource: null`, so there was no correct value on disk to reconcile against. A third: `effective_route.json` rewrote `selected_agent_id` without rewriting `selected_agent_name`, leaving `claude_code` labelled `"Antigravity CLI (agy)"`.
* **Fix:** the two meanings are now separate fields. `TaskSpec.effective_implementer_resource_id` is the single authoritative identity, written where implementation settles — on success, on promotion of a retained candidate, and on recovery after interruption — and never by any review, remediation or dispatch path; `actual_agent` mirrors it. `TaskSpec.dispatch_resource_id` is the transient per-invocation slot, read through the non-serialized `dispatch_target` property so implementation paths that construct a fresh `TaskSpec` per candidate are unaffected. The identity is pinned before the authority gate rather than only at Stage 8, and `selected_agent_name` now follows its id. Full attempt history is untouched.
* **Impact:** contained but real. Reviewer independence was **not** contaminated on this run, because the independence check reads `final_impl_resource_id` rather than `actual_agent`. But `actual_agent` is the durable, human- and machine-readable record of who implemented the change, and it is what an audit, a resume path, a metrics query, or any future policy keyed on the implementer would consult. It currently disagrees with the effective route and with the review evidence.
* **Deterministic reproduction:** run a governed task in which the first N-1 implementation attempts fail and the final attempt also fails while leaving a non-empty task-attributable delta (the PR #56 retained-salvage path), then compare `task.yaml`'s `actual_agent` against `effective_route.json`'s `selected_agent_id` and the `independence.implementer` recorded in `reviews/*/result.json`.
* **Deterministic acceptance:** after any governed run, `task_spec.actual_agent` names the resource that produced the retained or accepted candidate, and agrees with `effective_route.selected_agent_id` and with the implementer recorded in every review result.
* **Related observation from the same run, filed here rather than as its own item because it may be legitimate rather than defective:** `codex` completed a `simplicity-reviewer` review in 356.332s but hit the 600s budget on `test-falsifier` four times out of four (600.035-600.070s, `timed_out: true`, `timeout_source: harness`, `EXECUTION_BUDGET_EXCEEDED`). The `test-falsifier` brief may simply be heavier than one provider can finish inside the budget. This is recorded as evidence only; the review budget was deliberately not changed again in response to a single provider/role pair.
...
* **Confirmed 2026-08-30, with an additional structural finding:** the proposed fix was correct, and prose matching could not have worked here anyway. Devin's error is a multi-line JSON blob whose **last non-empty line is `}`**, and `_classify_terminal_hard_failure` only inspects the last non-empty line — so the quota text is structurally unreachable by the anchored tier regardless of which patterns are added to it. Measured directly against the live stderr before any change:

  ```
  terminal_hard_failure   -> None
  structured              -> None
  quota substring match   -> []      # "quota exhausted" never appears; the text is "quota has been exhausted"
  session substring match -> []      # "resource_exhausted" never appears; the markers spell it with a space
  ```
* **Fix:** a structured error-envelope reader (`_classify_structured_error_envelope`) that matches a quoted, vendor-prefixed `errorKind` key and maps its *value* — `resource_exhausted` → `QUOTA_EXHAUSTED`, `rate_limited` → `RATE_LIMITED`, and so on. It runs ahead of the anchored-prose tier and inside the structured tier, so a hard capacity stop still outranks an earlier denied tool. It reads only the provider's own error channels (structured metadata, `error_message`, `stderr`) and never `stdout`, which is an agent transcript and therefore arbitrary third-party content that could carry such a pair by coincidence or by injection. Providers that emit no envelope keep exactly their previous classification path. Devin's spellings were also added to `EXHAUSTION_PATTERNS["devin_cli"]`, which already carried them for `agy`.
* **Adjacent defect found while fixing this — a quota event never expired:** `QUOTA_EXHAUSTED` was absent from the set of failure classes granted a `retry_after`, so a quota-exhausted resource stayed unavailable until someone called `reset_resource`. A single historical quota event removed a provider from the pool permanently, which is the opposite of the intended pool semantics. Quota now expires on its own longer clock (`quota_cooldown_seconds`, default 6h) — distinct from the 300s transient cooldown because a quota is billed over a day or a week, so re-probing it on the transient clock would spend attempts proving what is already known.
* **Adjacent defect found while fixing this — dead providers spent the healthy providers' budget:** `invoke_reviewer_with_failover` sliced `candidates[:max_attempts]` *before* the loop while the availability skip happened *inside* it, so a provider already known to be exhausted consumed one of the two slots without being asked anything. With two dead providers at the head of the list a role failed outright while healthy independent providers sat untried. It is now a bounded pool traversal: `MAX_REVIEWER_LAUNCH_ATTEMPTS` bounds *launches*, not traversal; a known-unavailable resource is recorded with `consumed_launch_budget: false` and costs nothing; and a launch that turns out to be a capacity discovery refunds its budget, because discovering a provider is out of capacity is not a review opportunity — it said nothing about the change. A **timeout is deliberately not refunded**: a provider that launched, worked and ran out of clock did consume a real opportunity, and says nothing about its own availability. The traversal cannot spin — de-duplication is tracked separately from budget accounting, so total launches per role per cycle can never exceed the candidate set (verified: eight all-exhausted providers produce exactly eight launches and terminate).
...
* **Adjacent observation, now fixed and confirmed live:** `src/control_plane/synthesis/engine.py` called `build_reviewer_candidates` without an `implementer` argument, so PR #60's implementer-last ordering did not apply on the product-synthesis review path. `implementing_provider` was already in scope; the gap was one argument wide. Passing it changed real routing, and exposed that an existing test was protecting the defect:

  ```
  before:  security-reviewer   devin_cli(fail) -> codex(completed)    <- codex is the implementer
  after:   security-reviewer   devin_cli(fail) -> agy(completed)
  ```

  `test_reviewer_failover_recovers_on_second_candidate` asserted that all three roles completed via *distinct* providers. It passed because the implementer reviewed its own code — that self-review is what made the three providers distinct. The assertion was retargeted at the property that actually matters (`implementing_provider not in completed_providers`); with four providers, one of them the implementer and one scripted to fail, two independent providers remain to cover three roles, so one necessarily serves two. That is allowed. **Provider independence from the implementer and provider diversity among reviewers are different properties, and this is what conflating them costs.**
...
* **Decision taken (2026-08-30): options 1 and 3 together; option 2 explicitly rejected.** Approval resumes the governed workflow into the deterministic gate (option 1), and a plan that does not pass reaches a terminal state that does not read as verified (option 3). Option 2's "explicit, separately recorded override" was **not** built: overruling a failed deterministic gate is a different authority act from authorizing a workflow to continue, and bundling it into this fix would have created exactly the escape hatch the issue is about. If it is ever wanted it must be its own named operation with its own evidence and terminal semantics — `test_no_generic_verification_override_exists` fails if `approve` or `resume` ever grows an `override`, `force` or `skip_verification` parameter.
* **How "escalated before Stage 6" is detected:** from the verification evidence itself, not from a checkpoint stage. A plan whose `overall_status` is still `unverified` has not run, whatever else the run recorded. That is also the fail-closed reading for tasks serialized before this fix, and it means a task cannot be mislabelled by a checkpoint that drifted.
* **Terminal states:** `passed` completes under normal policy. `failed` reaches `failed`, the same state the ordinary Stage 6 gate uses, by the same evidence. Partial, errored or never-run reaches `blocked`, because an implementation that was not judged must not be recorded as judged. Task runs with no `verification_plan.json` at all — which no governed run produces, since routing writes one before any stage can escalate — still complete via the bounded-execution receipt path, but their summary records `no_plan` rather than claiming verification.
* **Structural consequence:** Stage 6 now *calls* the same gate rather than restating it, so one-pass verification and post-approval verification cannot reach different conclusions from identical evidence. This also removed a `python_src` duplication the change had introduced, rather than raising the ceiling.
...
- **Bug 18 — HowlPlane's own CI never exercised the real `howldream` import boundary (fixed 2026-09-12):** `NativeHowlDreamProvider` in `src/control_plane/howldream_runner.py` has always done a real `from howldream.contracts import ExplorationRequest` / `from howldream.engine import explore`, but every existing test (`tests/test_howldream_integration.py`) substituted `DeterministicTestExplorationProvider` instead — directly, or via `HOWLDREAM_PROVIDER=test` — so a break in the real `howldream` package would never fail CI. Tracked upstream as `howldream/issues.md` item 2. Fixed by adding `tests/test_howldream_live_integration.py` (marked `live`, gated by the existing `HOWLPLANE_LIVE_PROVIDERS`/`HOWLPLANE_RUN_LIVE_TESTS` scaffolding in `tests/conftest.py` and `.github/workflows/test.yml`'s `nightly-python` job) which drives `NativeHowlDreamProvider.explore()` against a real, pinned `howldream` install (`git+https://github.com/howlcipher/howldream.git@bd10b18b7182e5216b494d728d015dbbba9b32d0`, installed only in that CI step, never added to `pyproject.toml`) and validates the resulting envelope against a vendored copy of HowlDream's generated `howl.exploration_result/v1` JSON Schema (`contracts/howl/`). Verified locally against the real package before merging (3 tests passed) and confirmed the live tests are correctly deselected on every ordinary `pytest` invocation (0 selected without the env var). Does not add `howldream` as a runtime dependency; HowlPlane's native provider still degrades gracefully when it's absent.
