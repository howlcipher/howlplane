...
| 16 | [`task_spec.actual_agent` lags one implementation attempt behind the resource that actually produced the candidate](#16-task_specactual_agent-lags-one-implementation-attempt-behind-the-resource-that-actually-produced-the-candidate) | Pending | 4.0 (8×1÷2) | Sonnet 5 | Gemini 3 Pro | On HOWLFRAM-BUG-52 the durable task spec recorded `actual_agent: codex` (attempt 2) while the effective route, the retained attempt directory, the operator summary and all ten review independence records said `claude_code` (attempt 3). The independence machinery reads a different field and was unaffected, but the audit field disagrees with every other artifact. |
...
* **Root Cause (suspected, not yet confirmed):** `actual_agent` appears to be assigned during the failover loop but not refreshed for the final attempt when that attempt also fails and its artifact is retained as salvage. The orchestrator tracks the true producer separately in `final_impl_resource_id`, which is what reaches `ReviewRunner.execute_review_cycle` as `implementer_resource_id` — which is why review independence was computed correctly.
