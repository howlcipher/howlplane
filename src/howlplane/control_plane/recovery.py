...
from howlplane.control_plane.process_manager import ProcessTracker, ProcessRecord
from howlplane.control_plane.proposed_action import ProposedAction, infer_proposed_actions
from howlplane.control_plane.reconciliation import ReviewFinding, ReconciliationResult, ReviewReconciler
from howlplane.control_plane.review_runner import SingleReviewResult, parse_and_validate_findings
...
class CrashRecoveryEngine:
    ...
    def inspect_task(
        ...
        if is_process_running:
            classification = RetryClassification.HUMAN_DECISION_REQUIRED
            recommendation = (
                f"Task is actively running (PID {active_process.pid}). Run 'howlplane cancel {task_id}' to cancel."
            )
            ...
            recommendation = (
                f"HowlChangeOps action already executed natively. Run 'howlplane resume {task_id}' to reconcile receipt."
            )
            ...
            recommendation = (
                ...
                f"Wait for it to finish, or run 'howlplane cancel {task_id}'."
            )
            ...
            recommendation = (
                ...
                f"If that run is definitely gone, run 'howlplane unlock {task_id}' "
                f"first, then 'howlplane resume {task_id}'."
            )
            ...
            recommendation = (
                ...
                f"'howlplane unlock {repo_lock_task_id or task_id}' first, then "
                f"'howlplane resume {task_id}'."
            )
        else:
            classification = classify_stage_retry(last_stage)
            recommendation = f"Run 'howlplane resume {task_id}' to recover from {last_stage}."
    ...
    def reconcile_howlchangeops_receipt(
        ...
        """
        from howlplane.control_plane.executor import HowlChangeOpsExecutor
