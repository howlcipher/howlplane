...
BUILTIN_AGENTS: List[AgentProfile] = [
    ...
    AgentProfile(
        # Canonical identity is `cursor`; `agent` is only the executable name.
        agent_id="cursor",
        ...
        available_tools=["shell", "file_edit", "search"],
        reasoning_tier="tier_2",
        supports_repository_access=True,
        supports_command_execution=True,
        supports_long_running_tasks=False,
        ...
        availability="available",
        operator_preference=None,
        notes="Cursor's `agent` print mode; each new workspace must be trusted (see `howlplane factory prepare`).",
        interface_id="cursor_subscription_cli",
        supports_tool_calling=True,
    ),
    AgentProfile(
