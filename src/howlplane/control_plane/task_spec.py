...
class DataClassSerializationMixin:
    ...
    def save_to_file(self, file_path: Union[str, Path]) -> None:
        from howlplane.control_plane.atomic_io import atomic_write_yaml, atomic_write_json
        p = Path(file_path)
    ...
    def load_from_file(cls, file_path: Union[str, Path]):
        from howlplane.control_plane.atomic_io import safe_load_yaml, safe_load_json
        p = Path(file_path)
        if p.suffix in (".yaml", ".yml"):
