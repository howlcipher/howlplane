...
| **Workspace trust (`agents doctor --repo --live`, `factory prepare`)** | Explicit operator command | A no-op prompt with a deliberately invalid model name (the CLI rejects it before inference) and the workspace path; Cursor preparation adds its documented `--trust` flag | Cursor and Devin services | `connected` mode and an explicit command | Without `--live`, trust is read from local vendor files only; nothing is sent in `local_only` mode |
