...
* **Workspace** (with `--repo`): `READY`, `TRUST REQUIRED`, `UNKNOWN`, `UNSUPPORTED`, or `ERROR`, read from each vendor's own trust store without any prompt. With `--repo`, each agent also shows **Vendor trust** (the vendor's own verdict, or `NOT ENFORCED`), **Trust policy**, **Workspace** (effective readiness under that policy), and **Mechanism** (the audited flag a `bypass` invocation passes). `--workspace-trust {strict,prepare,bypass}` overrides the configured policy for this report. See [WORKSPACE_TRUST.md](WORKSPACE_TRUST.md#workspace-trust-policy).
...
Trust gating is effective readiness under the workspace trust policy: under `bypass`, an agent with an audited bypass flag is not gated, and the report lists it under "Trust check bypassed". Trust-gated agents are not workers in the Factory worktree. If some are gated and others can still implement and review, the campaign is `DEGRADED`. If every implementation-capable agent is gated, it is `BLOCKED`.
