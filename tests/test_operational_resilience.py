...
def test_ambiguous_lock_fails_closed_but_is_recoverable(tmp_path):
    ...
    assert "howlplane unlock TASK-AMBIG" in message
    assert lock_path.exists()
...
def test_recovery_does_not_recommend_resume_behind_a_held_lock(tmp_path):
    ...
    assert "howlplane unlock TASK-RECO" in diag["recommendation"]
    ...
    try:
        ...
        assert "howlplane resume" not in diag["recommendation"]
    finally:
        owner.kill()
        owner.wait()
